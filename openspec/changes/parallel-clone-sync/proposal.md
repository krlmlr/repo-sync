...
- **The inventory is worked on several repositories at a time.**
  ...
  eight jobs by default, `REPO_SYNC_JOBS` to choose another number.
...
- **`scripts/lib.sh`** — the fan-out, the failure log, the parallel check,
...
- **Existing mirrors**: nothing to migrate.
  The same git commands run against the same directories, in a different order.

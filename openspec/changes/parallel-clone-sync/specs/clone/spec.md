...
The system SHALL work on more than one repository at once,
since each mirror's clone or update is independent of every other's and spends most of its duration waiting on the network.
...
- **THEN** that repository's output is printed as one block,
  not split across the output of the repositories running beside it
...
— both its checkout and its bare mirror —

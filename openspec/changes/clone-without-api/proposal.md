...
- **`gh` is no longer used at all.**
  ...
  no helper to configure,
  and nothing token-shaped to leak into a remote URL or a process argument list.
  The prerequisite becomes SSH access to GitHub,
  in place of `gh auth login`.
- **Normalise `origin` on every run.**
  A mirror cloned before this change carries an HTTPS `origin`;
  `clone` rewrites it to the SSH URL,
  exactly as it already normalises the `template` remote.
  Which transport a mirror speaks should follow from the inventory,
  not from the day it was created.
- Forks lose the `upstream` remote
  that `gh repo clone` adds
...
- `env-setup`: the prerequisite is SSH access to GitHub
  rather than an authenticated `gh`.
...
- **`scripts/sync.sh`** — unchanged.
  It fetches and rebases through remotes that are already configured,
...
- `scripts/fetch_inventory.py`,
  which reads a public branch list with an anonymous `git ls-remote` over HTTPS.
...
- Rewriting the `template` remote,
  which is a relative local path and has no transport to choose.

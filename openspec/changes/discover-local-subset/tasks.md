...
- [ ] 1.1 Add `scripts/discover_local.py`
          that reads `repos.yml` and validates exactly one `template: true` entry
          (exit non-zero otherwise, before emitting anything)
- [ ] 1.2 Implement an origin-remote resolver
          that normalizes SSH and HTTPS URLs
          (with/without trailing `.git`, case-insensitive) to `<org>/<repo>`
- [ ] 1.3 Scan the base directory for candidates in both layouts:
          flat children of the base dir, and `mirrors/<org>/<repo>/`
- [ ] 1.4 Match a candidate only when basename == `repo`
          AND resolved `origin` == `<org>/<repo>`;
          disambiguate same-named repos across orgs via origin
- [ ] 1.5 Mark the discovered `template: true` entry as the template;
          warn (continue) when it is absent locally
- [ ] 1.6 Report unrecognized extras and origin mismatches;
          exit zero whenever a valid subset is found
...
- [ ] 2.1 Add a named `mise` task for discovery in `mise.toml`, consistent with `fetch-inventory` and `clone`
- [ ] 2.2 Confirm `mise run <task>` executes the script with the pinned Python interpreter and supports a non-default base directory

## 3. repo-sync AGENTS.md

- [ ] 3.1 Add `AGENTS.md` at the repo root
          describing the repo purpose (mirror inventory, reconcile vs template, push back)
...
- [ ] 3.4 Reference the discovery capability
          so the doc stays accurate to this change
...
- [ ] 4.3 Verify an extra sibling not in `repos.yml` is reported
          and discovery still exits zero

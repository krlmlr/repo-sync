...
- **THEN** each `mirrors/<org>/<repo>/` directory matching an inventory entry is reported as discovered
...
- **THEN** the reported `<org>/<repo>` identity is identical regardless of layout
...
- **WHEN** a directory named `dm` has an `origin` remote resolving to `cynkra/dm` and `repos.yml` contains `cynkra/dm`
...
- **THEN** the system exits zero after emitting the subset, regardless of extras or a missing template
...
- **THEN** each emitted entry includes `org`, `repo`, local path, and template flag
...
The system SHALL expose discovery as a named `mise` task
so contributors run `mise run <task>` without knowing the underlying script path or interpreter,
...
- **WHEN** the discovery task is run with a base directory other than the default

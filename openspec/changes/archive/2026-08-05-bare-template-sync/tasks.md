...
- [x] 1.2 Move the `repos.yml` reader and the exactly-one-`template: true` rule into `load_inventory`,
      setting `template_slug` and `ordered_slugs` with the template first
...
- [x] 2.1 In `scripts/clone.sh`,
      clone the `template: true` entry a second time as `mirrors/<org>/<repo>.git` with `gh repo clone ... -- --mirror`
...
- [x] 4.4 Record a failure, naming `mise run clone`, for a mirror that exists without a `template` remote,
      and for a missing or non-bare template mirror
...
- [x] 5.1 Fresh clone against a fixture:
      both mirrors of the template exist, one bare and one not, and every `template` remote points at the bare one
...
- [x] 5.7 `sync` skips a mirror that is not cloned,
      and exits non-zero for a missing `template` remote, a missing bare mirror, and an inventory with no `template: true`

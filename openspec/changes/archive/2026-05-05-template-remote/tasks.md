...
- [x] 2.1 In `scripts/fetch_inventory.py`, read existing `repos.yml` (if present)
  and capture all `(org, repo)` tuples carrying `template: true`
...
- [x] 3.1 In `scripts/clone.sh`, parse `repos.yml` for the unique `template: true` entry;
  fail fast on zero or multiple matches before processing any mirror
- [x] 3.2 Process the template entry first (clone or update), then iterate over the remaining entries
- [x] 3.3 After a successful clone or fetch on a non-template mirror,
  add or update a `template` remote pointing at the local relative path `../../<template-org>/<template-repo>`
  using `git remote add` / `git remote set-url`

...
pointing at the local relative path `../../<template-org>/<template-repo>` (resolving to the template's mirror directory under `mirrors/`).
...
The system SHALL clone or update the entry flagged `template: true` before processing any non-template entry,
...
- **THEN** the template entry is cloned before any non-template entry,
  so each subsequent `git remote add template ../../<template-org>/<template-repo>` resolves to an existing directory

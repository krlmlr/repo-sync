...
The template's own mirror SHALL be skipped,
as it carries no `template` remote.

The fetch SHALL suppress tags itself rather than relying on the remote's configuration:
`clone` writes that configuration,
`sync` is the command that runs every day,
and a mirror wired up before the configuration existed must not keep importing tags
until someone happens to run the other command.

#### Scenario: Template refs updated

- **WHEN** a non-template mirror has a `template` remote
  and the bare mirror has moved
...
- **THEN** `sync` removes none of them,
  since it keeps local state that has not been pushed;

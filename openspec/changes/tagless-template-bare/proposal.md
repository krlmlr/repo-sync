...
`--no-tags` passed again by `sync`, and `--prune-tags` to sweep up what already leaked.
...
so the remaining sync operations become plain git commands that any tool iterating over the checkouts can run.
...
- Stop writing `remote.template.tagOpt`, and remove it where earlier runs wrote it, once the bare on disk is verifiably tagless.
...
- Move the bare's normalisation and refresh into `lib.sh`,
  so `clone` and `sync` share one implementation and every command re-establishes the invariant before any mirror fetches from it.

## Capabilities

### New Capabilities
<!-- None: the bare mirror and the `template` remote are already owned by the `clone`, `template-remote` and `sync` capabilities. -->

### Modified Capabilities
- `clone`: how the template's bare mirror is built and normalised,
  and the `template` remote configured on every mirror rather than only the non-template ones.
- `template-remote`: the no-tags guarantee moves from per-mirror configuration to the bare repository itself,
  and the remote is carried by every mirror.
...
- **`scripts/clone.sh`**: `clone_or_update_bare` rewritten;
  `configure_template_remote` loses its template special case and its `tagOpt` write.
...
- **Existing `mirrors/` trees**: normalised in place on the next `clone` or `sync` run.
  No re-clone and no extra network beyond the usual fetch.
...
- Repointing the template checkout's `origin` at the bare. `origin` stays on GitHub; the checkout gains the `template` remote alongside it.

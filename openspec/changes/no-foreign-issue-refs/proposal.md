...
a squash merge takes the pull request's title as the subject and appends `(#12)`,
...
once per mirror, on forty-seven unrelated issues.
...
so the template's history carries these references whatever anyone does upstream.
The moment to intervene is the one where a commit is copied into a mirror.
...
- **Repair what can be repaired, refuse what cannot.**
  ...
  which points at the issue the message meant and links to it from anywhere.
...
- `template-remote`: A commit copied from the template carries no reference
